...
function createKeywords(env = process.env) {
  ...
  // Prefer Ahrefs (the chosen provider) when its token is set. Lazy-require to
  // avoid a circular-init issue: ahrefs.js pulls clusterKeywords from this file.
  if (source.AHREFS_API_TOKEN) {
    const { createAhrefs } = require("./ahrefs");
    const ahrefs = createAhrefs(source);
  ...
  return {
    async research(
      ...
      if (seeds.length) {
        ...
        dfsCalls += 1;
        if (response.ok === false) throw new Error(`DataForSEO request failed (${response.status}).`);
