...
function httpCompleter({ provider, apiKey, model }) {
...
function defaultCodexRun(bin, args, stdin) {
  return new Promise((resolve, reject) => {
    ...
    child.on("close", (code) => {
      if (code === 0) resolve();
      else reject(new Error(`codex exec exited ${code}: ${stderr.slice(-300)}`));
    });
    if (child.stdin) {
...
function codexCompleter({ bin, model }) {
...
function createLlm(env = process.env) {
  ...
  if (provider === "codex") {
    ...
    complete = codexCompleter({ bin: env.SIR_BLOGGS_CODEX_BIN || "codex", model: env.SIR_BLOGGS_LLM_MODEL || "" });
  } else {
    ...
    complete = httpCompleter({ provider, apiKey, model });
  }
