...
function createLlm(env = process.env) {
  ...
  async function fullArticle({ title, outline, keyword, siteContext }, { fetchImpl = fetch } = {}) {
    const system = "You are a professional writer who produces publish-ready HTML article sections.";
    const prompt = [
      `Write the full article "${title}" targeting "${keyword}".`,
      "Use the outline below. Each section's content must be HTML (<p>, <ul>, <h3> as needed).",
      '{"sections": [{"title": "...", "content": "<p>...</p>"}], "metaDescription": "..."}',
      outline ? `Outline:\n${JSON.stringify(outline)}` : "",
      contextText({ ...siteContext, keyword }),
    ].filter(Boolean).join("\n\n");
    const data = await complete({ system, prompt, json: true, maxTokens: 2500, temperature: 0.7 }, { fetchImpl });
    const sections = Array.isArray(data.sections) ? data.sections : [];
    return {
      sections: sections.map((section) => ({
        title: String((section && section.title) || "").trim(),
