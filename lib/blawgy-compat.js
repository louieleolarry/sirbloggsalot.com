...
// Single-site deployment default. Threaded through the shim (blawgy-app.html, injected
// by server.js) and these backend defaults so the whole install operates on one site.
// (Per-account multi-tenant domains is a later feature.)
const PRIMARY_SITE = runtime.SIR_BLOGGS_PRIMARY_SITE || "sirbloggsalot.com";
...
function createBlawgyCompat({ root, readSessionUser, readRequestJson, sendJson, publicUser, providers }) {
  ...
  // --- Phase 2: GSC OAuth stash + token-at-rest crypto -----------------------
  // Short-lived, single-use maps for the OAuth dance (never persisted). state ties
  // the Google redirect back to the initiating user+site; selectionToken carries the
  // fetched tokens+properties between the callback and the user's property choice.
  const gscStateMap = new Map();
  const gscPendingMap = new Map();
  const GSC_STASH_TTL_MS = 10 * 60 * 1000;
  function gscSweep(map) {
    const cutoff = Date.now() - GSC_STASH_TTL_MS;
    for (const [k, v] of map) if (!v || v.at < cutoff) map.delete(k);
  }
  function gscTokenKey() {
    const raw = runtime.SIR_BLOGGS_GSC_TOKEN_KEY;
    return raw ? crypto.createHash("sha256").update(String(raw)).digest() : null;
  }
  function gscEncrypt(text) {
    const key = gscTokenKey();
    if (!key || !text) return text || "";
    const iv = crypto.randomBytes(12);
    const cipher = crypto.createCipheriv("aes-256-gcm", key, iv);
    const enc = Buffer.concat([cipher.update(String(text), "utf8"), cipher.final()]);
    return `gcm:${iv.toString("base64")}:${cipher.getAuthTag().toString("base64")}:${enc.toString("base64")}`;
  }
  function gscDecrypt(value) {
    if (typeof value !== "string") return value || "";
    if (!value.startsWith("gcm:")) return value; // plaintext (no key configured) or empty
    const key = gscTokenKey();
    if (!key) return "";
    try {
  ...
  function requestOrigin(req) {
    ...
    return `${proto}://${req.headers.host || "sirbloggsalot.com"}`;
  }
  ...
  function cleanSite(value) {
    const raw = String(value || PRIMARY_SITE).trim() || PRIMARY_SITE;
    return raw
      .replace(/^https?:\/\//i, "")
      .replace(/^www\./i, "")
      .split(/[/?#]/)[0]
  ...
  function defaultUserState(user) {
    const siteName = cleanSite(PRIMARY_SITE);
  ...
  function ensureSite(account, requestedSite, user) {
    const siteName = cleanSite(requestedSite || Object.keys(account.sites)[0] || PRIMARY_SITE);
  ...
  function publicSiteSettings(site) {
    const g = site.settings.gsc;
    const gsc = (g && (g.connected || g.access_token))
      ? {
          connected: true,
          access_token: "connected", // redacted marker; the real token never leaves the server
          connected_at: g.connected_at || null,
          connected_site: g.connected_site || g.property || `sc-domain:${site.site}`,
          propertyUrl: g.propertyUrl || g.property || g.connected_site || `sc-domain:${site.site}`,
        }
      : null;
    const settings = {
      apiToken: "", // write-only; presence flagged via hasApiToken
      appPassword: "", // write-only; presence flagged via hasAppPassword
      ...
      hasApiToken: Boolean(site.settings.apiToken),
      hasAppPassword: Boolean(site.settings.appPassword),
      hasAuthToken: Boolean(site.settings.authToken),
      hasShopifyClientSecret: Boolean(site.settings.shopifyClientSecret),
      hasFramerApiKey: Boolean(site.settings.framerApiKey),
      hasGithubToken: Boolean(site.settings.githubToken),
    };

    // Non-secret identifiers only — secrets (authToken, shopifyClientSecret,
    // framerApiKey, appPassword, apiToken, githubToken) are write-only.
    for (const key of [
      "categoryId",
      "framerCollectionId",
      "framerProjectUrl",
  ...
  async function routeAccount(req, res, url, user) {
    ...
    const accountResult = await withAccount(user, (account) => {
      ...
      if (req.method === "POST" && url.pathname === "/update-site-settings") {
        ...
        // Never let an empty/omitted secret field wipe a stored credential — the SPA
        // renders these write-only and submits "" when the user hasn't re-typed them.
        for (const k of ["apiToken", "appPassword", "authToken", "shopifyClientSecret", "framerApiKey", "githubToken"]) {
          if (incoming[k] === "" || incoming[k] === undefined || incoming[k] === null) delete incoming[k];
        }
        site.settings = { ...site.settings, ...incoming, site: site.site };
        return { status: 200, payload: { success: true, settings: publicSiteSettings(site).settings } };
      }
  ...
  function routeKeywordResearch(req, url, parts, body, account, user) {
    if (req.method === "POST" && parts[1] === "onboarding-preview") {
      const previewSite = cleanSite(body.website || body.site || bodySiteFromPreview(url) || PRIMARY_SITE);
  ...
  async function runArticleJob({ user, siteName, entryId }) {
    ...
    // If a CMS publisher is configured for this site, push the article now (outside
    // the store lock). No publisher/creds -> keep today's local "published" behavior.
    let publishResult = null;
    let publishAttempted = false;
    const pub = activeProviders.publishers;
    const blogType = String((snapSite.settings && snapSite.settings.blogType) || "").toLowerCase();
    const publisher = pub && pub.getPublisher(blogType);
    const conn = publisher && pub.connFromSettings(blogType, snapSite.settings, snapSite.site);
    if (publisher && conn && publisher.hasCreds(conn)) {
      publishAttempted = true;
      const postArticle = {
        title: snapEntry.title, html, excerpt: article.metaDescription || "",
        slug: slugFromTitle(snapEntry.title), status: snapSite.settings.draft ? "draft" : "publish",
        date: snapEntry.publishDate || null, tags: keyword ? [keyword] : [], categories: [], featuredImageUrl: null,
      };
      try {
        publishResult = snapEntry.cmsPostId
          ? await publisher.update(snapEntry.cmsPostId, postArticle, conn)
          : await publisher.publish(postArticle, conn);
      } catch (error) {
        providerLog("/generate-blog publish", error);
      }
    }
    ...
    await withAccount(user, (account) => {
      ...
      entry.hasContent = true;
      if (publishResult) {
        entry.cmsPostId = publishResult.cmsPostId;
        entry.publishedUrl = publishResult.publishedUrl || entry.publishedUrl || null;
        entry.blogStatus = publishResult.status === "draft" ? "draft" : "published";
        entry.publishTarget = blogType;
      } else if (publishAttempted) {
        entry.blogStatus = "draft"; // content generated but CMS publish failed
        entry.generationError = "Article generated; CMS publish failed (saved as draft).";
      } else {
        entry.blogStatus = "published"; // local-only (no CMS configured)
      }
      entry.status = entry.blogStatus;
      entry.published = entry.blogStatus === "published";
      entry.updatedAt = iso();
      return null;
    });
  }
  ...
  async function handleKeywordResearch(req, res, url, user) {
    ...
    if (req.method === "POST" && seg2 === "onboarding-preview") {
      const body = await bodyFor(req);
      const previewSite = {
        site: cleanSite(body.website || body.site || PRIMARY_SITE),
  ...
  async function persistGscConnection(user, blawgySite, property, tokens) {
    let out = null;
    await withAccount(user, (account) => {
      const site = ensureSite(account, blawgySite, user);
      site.settings.gsc = {
        connected: true,
        property,
        connected_site: property,
        propertyUrl: property,
        property_type: property.startsWith("sc-domain:") ? "sc-domain" : "url-prefix",
        connected_at: iso(),
        access_token_enc: gscEncrypt(tokens.access_token),
        refresh_token: gscEncrypt(tokens.refresh_token),
        expiry: tokens.expiry,
      };
      out = publicSiteSettings(site).settings.gsc;
      return null;
    });
    return out;
  }

  // Real Google Search Console. Returns false to fall through to the deterministic
  // placeholder /gsc/* handlers when the OAuth provider isn't configured / not connected.
  async function handleGscRoute(req, res, url, user) {
    if (!activeProviders.gsc) return false;
    const gsc = activeProviders.gsc;
    const pathName = url.pathname;
    const method = req.method;
    const redirect = (loc) => { res.writeHead(302, { location: loc, "cache-control": "no-store" }); res.end(); return true; };
    const back = (q) => redirect(`/reports?${q}`);

    if (method === "GET" && pathName === "/gsc/connect/start") {
      gscSweep(gscStateMap);
      const state = crypto.randomBytes(24).toString("base64url");
      gscStateMap.set(state, { site: cleanSite(firstParam(url, "site")), userId: user.id || user.email, at: Date.now() });
      return redirect(gsc.authorizeUrl({ redirectUri: `${requestOrigin(req)}/gsc/oauth2callback`, state }));
    }

    if (method === "GET" && pathName === "/gsc/oauth2callback") {
      const oauthError = firstParam(url, "error");
      if (oauthError) return back(`error=${encodeURIComponent(oauthError)}`);
      const code = firstParam(url, "code");
      if (!code) return back("error=invalid_request");
      // Which SBA site to attach the connection to: the server-initiated flow
      // (/gsc/connect/start) carries it in a signed state; the vendored SPA builds its
      // OWN authorize URL and doesn't, so fall back to the logged-in user's current
      // site. The session cookie (requireUser) already gates this endpoint.
      gscSweep(gscStateMap);
      const state = firstParam(url, "state");
      const pending = state ? gscStateMap.get(state) : null;
      if (pending) gscStateMap.delete(state);
      let blawgySite = pending && pending.userId === (user.id || user.email) ? pending.site : null;
      if (!blawgySite) {
        const snap = await readAccountSnapshot(user);
        blawgySite = cleanSite(Object.keys((snap && snap.sites) || {})[0] || PRIMARY_SITE);
      }
      try {
        const tokens = await gsc.exchangeCode({ code, redirectUri: `${requestOrigin(req)}/gsc/oauth2callback` });
        const sites = await gsc.listSites(tokens.access_token);
        providerLog("/gsc/oauth2callback ok", `site=${blawgySite} properties=${sites.length}`);
        if (!sites.length) return back("error=no_properties");
        if (sites.length === 1) {
          await persistGscConnection(user, blawgySite, sites[0].url, tokens);
          return back("connected=true");
        }
        gscSweep(gscPendingMap);
        const selectionToken = crypto.randomBytes(24).toString("base64url");
        gscPendingMap.set(selectionToken, { tokens, sites, userId: (user.id || user.email), site: blawgySite, at: Date.now() });
        return back(`select_domain=true&token=${encodeURIComponent(selectionToken)}`);
      } catch (error) {
        providerLog("/gsc/oauth2callback", error);
        return back("error=exchange_failed");
      }
    }

    if (method === "GET" && pathName.startsWith("/gsc/sites/")) {
      gscSweep(gscPendingMap);
      const pending = gscPendingMap.get(decodeURIComponent(pathName.slice("/gsc/sites/".length)));
      if (!pending) { sendJson(res, 404, { success: false, message: "Selection expired. Reconnect Search Console." }); return true; }
      sendJson(res, 200, { success: true, sites: pending.sites });
      return true;
    }

    if (method === "POST" && pathName === "/gsc/connect") {
      const body = await bodyFor(req);
      gscSweep(gscPendingMap);
      const pending = gscPendingMap.get(String(body.token || ""));
      if (!pending) return false; // no real pending OAuth session -> placeholder path
      const selected = String(body.selectedGscSite || body.gscSite || body.siteUrl || "").trim();
      if (!pending.sites.some((s) => s.url === selected)) { sendJson(res, 400, { success: false, message: "Unknown property." }); return true; }
      const gscPublic = await persistGscConnection(user, cleanSite(body.currentBlawgySite || pending.site), selected, pending.tokens);
      gscPendingMap.delete(String(body.token));
      sendJson(res, 200, { success: true, gsc: gscPublic });
      return true;
    }

    if (method === "POST" && pathName === "/gsc/disconnect") {
      const body = await bodyFor(req);
      let refresh = "";
      await withAccount(user, (account) => {
        const site = ensureSite(account, body.site, user);
        refresh = gscDecrypt(site.settings.gsc && site.settings.gsc.refresh_token) || "";
        site.settings.gsc = null;
        return null;
      });
      if (refresh) { try { await gsc.revoke(refresh); } catch (_) { /* best effort */ } }
      sendJson(res, 200, { success: true, gsc: null });
      return true;
    }

    if (method === "GET" && pathName === "/gsc/data") {
      const snapshot = await readAccountSnapshot(user);
      const site = snapshot.sites && snapshot.sites[cleanSite(firstParam(url, "site"))];
      const conn = site && site.settings && site.settings.gsc;
      if (!conn || !conn.property) return false; // not really connected -> placeholder/500
      let accessToken = gscDecrypt(conn.access_token_enc);
      if (!conn.expiry || conn.expiry < Date.now() + 60000) {
        try {
          const refreshed = await gsc.refresh({ refresh_token: gscDecrypt(conn.refresh_token) });
          accessToken = refreshed.access_token;
          await withAccount(user, (account) => {
            const s = ensureSite(account, firstParam(url, "site"), user);
            if (s.settings.gsc) { s.settings.gsc.access_token_enc = gscEncrypt(refreshed.access_token); s.settings.gsc.expiry = refreshed.expiry; }
            return null;
          });
        } catch (error) {
          providerLog("/gsc/data refresh", error);
          sendJson(res, 401, { success: false, needsReconnect: true, message: "Search Console session expired. Please reconnect." });
          return true;
        }
      }
      const dims = allParams(url, ["dimensions", "dimension", "filters[dimensions]", "filters[dimensions][]", "filters[dimensions][0]"]);
      const dimension = dims.find((d) => ["date", "page", "query"].includes(d)) || "date";
      const rowLimit = Number(firstParam(url, "limit") || firstParam(url, "filters[limit]")) || 100;
      const endDate = firstParam(url, "filters[endDate]") || firstParam(url, "endDate") || iso().slice(0, 10);
      const startDate = firstParam(url, "filters[startDate]") || firstParam(url, "startDate") || iso(-28).slice(0, 10);
      try {
        const rows = await gsc.searchAnalytics(accessToken, conn.property, { startDate, endDate, dimensions: [dimension], rowLimit });
        sendJson(res, 200, { success: true, rows, connectedSite: conn.connected_site || conn.property });
        return true;
      } catch (error) {
        providerLog("/gsc/data", error);
        if (error && error.status === 401) { sendJson(res, 401, { success: false, needsReconnect: true, message: "Reconnect Search Console." }); return true; }
        return false;
      }
    }
  ...
  function mergeCmsCreds(settings, blogType, body, siteDomain) {
    const next = { ...settings, blogType, site: siteDomain };
    const setIf = (k, v) => { if (v !== undefined && v !== null && v !== "") next[k] = v; };
    if (blogType === "wordpress") { setIf("username", body.username); setIf("appPassword", body.appPassword); }
    if (blogType === "shopify") { setIf("authToken", body.authToken); setIf("siteName", body.siteName); setIf("categoryId", body.categoryId); setIf("shopifyClientId", body.shopifyClientId); setIf("shopifyClientSecret", body.shopifyClientSecret); setIf("author", body.author); }
    if (blogType === "webflow") { setIf("apiToken", body.apiToken); setIf("collectionId", body.collectionId); setIf("fields", body.fields); }
    if (blogType === "astro") { setIf("githubToken", body.githubToken); setIf("repoOwner", body.repoOwner); setIf("repoName", body.repoName); setIf("branch", body.branch); setIf("contentDir", body.contentDir); setIf("commitMode", body.commitMode); }
    return next;
  }

  function articleForPublish(entry, site) {
    return {
      title: entry.title,
      html: entry.blogContent || "",
      excerpt: entry.articleSummary || "",
      slug: slugFromTitle(entry.title),
      status: site.settings.draft ? "draft" : "publish",
      date: entry.publishDate || null,
      tags: entry.keyword ? [entry.keyword] : [],
      categories: [],
      featuredImageUrl: entry.imageUrl || null,
    };
  }

  // Real CMS publishing. Returns false to fall through to the local placeholder
  // (test-connection always-success / fabricated publishedUrl) when no publisher or
  // no per-site creds — preserving graceful degradation.
  async function handleCmsRoute(req, res, url, user, pathName) {
    const pub = activeProviders.publishers;
    if (!pub) return false;

    if (req.method === "GET" && pathName === "/shopify/blogs") {
      const snap = await readAccountSnapshot(user);
      const site = snap.sites && snap.sites[cleanSite(firstParam(url, "site"))];
      const publisher = pub.getPublisher("shopify");
      const conn = pub.connFromSettings("shopify", site && site.settings, site && site.site);
      if (!site || !publisher || !publisher.hasCreds(conn)) return false;
      try {
        const { blogs = [], authors = [] } = await publisher.validate(conn);
        sendJson(res, 200, { success: true, data: blogs, authors });
        return true;
      } catch (error) { providerLog("/shopify/blogs", error); return false; }
    }

    if (req.method === "GET" && pathName === "/webflow/fields") {
      const snap = await readAccountSnapshot(user);
      const site = snap.sites && snap.sites[cleanSite(firstParam(url, "site"))];
      const publisher = pub.getPublisher("webflow");
      const conn = pub.connFromSettings("webflow", site && site.settings, site && site.site);
      if (!site || !publisher || !publisher.hasCreds(conn)) return false;
      try {
        const fields = await publisher.listFields(conn);
        sendJson(res, 200, { success: true, fields });
        return true;
      } catch (error) { providerLog("/webflow/fields", error); return false; }
    }

    if (req.method === "POST" && pathName === "/test-connection") {
      const body = await bodyFor(req);
      const blogType = String(body.blogType || "").toLowerCase();
      const publisher = pub.getPublisher(blogType);
      if (!publisher) return false; // framer/wix/etc -> placeholder
      await withAccount(user, (account) => {
        const site = ensureSite(account, body.site, user);
        site.settings = mergeCmsCreds(site.settings, blogType, body, site.site);
        return null;
      });
      const snap = await readAccountSnapshot(user);
      const site = snap.sites && snap.sites[cleanSite(body.site)];
      const conn = pub.connFromSettings(blogType, site && site.settings, site && site.site);
      if (!publisher.hasCreds(conn)) { sendJson(res, 400, { success: false, message: "Missing required connection fields." }); return true; }
      try {
        const result = await publisher.validate(conn);
        const extra = {};
        if (blogType === "shopify") { extra.blogs = result.blogs || []; extra.authors = result.authors || []; }
        if (blogType === "webflow") { extra.fields = result.fields || []; }
        sendJson(res, 200, { success: true, message: "Connection successful.", ...extra });
        return true;
      } catch (error) {
        providerLog("/test-connection", error);
        const status = error && [401, 403, 404].includes(error.status) ? error.status : 400;
        sendJson(res, status, { success: false, message: (error && error.message) || "Connection failed." });
        return true;
      }
    }

    if (req.method === "POST" && ["/publish-draft", "/republish-article"].includes(pathName)) {
      const body = await bodyFor(req);
      const snap = await readAccountSnapshot(user);
      const site = (snap.sites && snap.sites[cleanSite(body.site)]) ||
        (snap.sites && Object.values(snap.sites).find((s) => findEntry(s, body.id)));
      const entry = site && findEntry(site, body.id);
      if (!site || !entry) return false;
      const blogType = String(site.settings.blogType || "").toLowerCase();
      const publisher = pub.getPublisher(blogType);
      const conn = pub.connFromSettings(blogType, site.settings, site.site);
      if (!publisher || !publisher.hasCreds(conn)) return false; // -> local placeholder publish
      try {
        const result = entry.cmsPostId
          ? await publisher.update(entry.cmsPostId, articleForPublish(entry, site), conn)
          : await publisher.publish(articleForPublish(entry, site), conn);
        await withAccount(user, (account) => {
          const s = account.sites && account.sites[cleanSite(site.site)];
          const e = s && findEntry(s, body.id);
          if (!e) return null;
          e.cmsPostId = result.cmsPostId;
          e.publishedUrl = result.publishedUrl || e.publishedUrl || null;
          e.blogStatus = result.status === "draft" ? "draft" : "published";
          e.status = e.blogStatus;
          e.published = e.blogStatus === "published";
          e.publishedAt = iso();
          e.publishTarget = blogType;
          if (pathName === "/republish-article") e.lastRepublishedAt = iso();
          e.updatedAt = iso();
          return null;
        });
        sendJson(res, 200, { success: true, article: { id: entry.id, blogStatus: result.status === "draft" ? "draft" : "published", publishedUrl: result.publishedUrl }, publishedUrl: result.publishedUrl, cmsPostId: result.cmsPostId });
        return true;
      } catch (error) {
        providerLog(pathName, error);
        sendJson(res, 502, { success: false, message: `Publish failed: ${(error && error.message) || "unknown error"}` });
        return true;
      }
    }
  ...
  // Real rankings from GSC (the "Google (your data)" source). Returns false so the
  // deterministic rankingRows() placeholder still answers when GSC isn't connected.
  async function handleGscRankings(req, res, url, user) {
    if (!activeProviders.gsc) return false;
    const parts = splitPath(url); // ["api","keyword-research",<site>,"rankings"]
    const siteName = cleanSite(parts[2]);
    const snapshot = await readAccountSnapshot(user);
    const site = snapshot.sites && snapshot.sites[siteName];
    const conn = site && site.settings && site.settings.gsc;
    if (!conn || !conn.property) return false;
    let accessToken = gscDecrypt(conn.access_token_enc);
    if (!conn.expiry || conn.expiry < Date.now() + 60000) {
      try {
        const refreshed = await activeProviders.gsc.refresh({ refresh_token: gscDecrypt(conn.refresh_token) });
        accessToken = refreshed.access_token;
        await withAccount(user, (account) => {
          const s = ensureSite(account, siteName, user);
          if (s.settings.gsc) { s.settings.gsc.access_token_enc = gscEncrypt(refreshed.access_token); s.settings.gsc.expiry = refreshed.expiry; }
          return null;
        });
      } catch (error) { providerLog("gsc rankings refresh", error); return false; }
    }
    try {
      const rows = (await activeProviders.gsc.searchAnalytics(accessToken, conn.property, { startDate: iso(-28).slice(0, 10), endDate: iso().slice(0, 10), dimensions: ["query"], rowLimit: 50 }))
        .map((r) => ({ keyword: (r.keys && r.keys[0]) || "", position: Math.round((r.position || 0) * 10) / 10, previousPosition: null, volume: r.impressions || 0, clicks: r.clicks || 0, url: null, source: "gsc" }));
      sendJson(res, 200, { success: true, rows, source: "gsc", asOf: iso(), availableSources: ["local", "gsc"], stale: false, truncated: false });
      return true;
    } catch (error) {
      providerLog("gsc rankings", error);
      return false;
    }
  }
  ...
  async function serveProviderRoute(req, res, url, user) {
    ...
    const method = req.method;
    try {
      if (pathName.startsWith("/gsc/")) { if (await handleGscRoute(req, res, url, user)) return true; }
      if (["/shopify/blogs", "/webflow/fields", "/test-connection", "/publish-draft", "/republish-article"].includes(pathName)) {
        if (await handleCmsRoute(req, res, url, user, pathName)) return true;
      }
      ...
      if (method === "GET" && pathName.startsWith("/api/keyword-research/") && pathName.endsWith("/rankings") && firstParam(url, "source") === "gsc") {
        if (await handleGscRankings(req, res, url, user)) return true;
      }
