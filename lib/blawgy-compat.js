...
function createBlawgyCompat({ root, readSessionUser, readRequestJson, sendJson, publicUser, providers }) {
  ...
  async function bodyFor(req) {
    // Cache the parsed body on the request so it can be read more than once
    // (the provider layer may inspect it, then fall through to a placeholder
    // handler that reads it again). The underlying stream can only be consumed
    // once.
    if (req.__blawgyBody !== undefined) return req.__blawgyBody;
    let parsed = {};
  ...
  async function handleGenerateBlog(req, res, url, user) {
    ...
    enqueueArticleJob({ user, siteName, entryId });
