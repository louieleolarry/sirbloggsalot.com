...
function createBlawgyCompat({ root, readSessionUser, readRequestJson, sendJson, publicUser }) {
  ...
  function defaultClusters(siteName, config = {}) {
  ...
  function makePlanEntry(keyword, patch = {}) {
    ...
    return {
      ...
      seoMetrics: {
        ...
        targetKeyword: keyword,
        searchVolume: Number(patch.searchVolume) || 500,
        difficulty: Number(patch.difficulty) || 20,
      ...
      why: patch.why || {
        aim: Object.prototype.hasOwnProperty.call(patch, "aim") ? patch.aim : null,
        factors: [`This topic gives ${cleanSite(patch.site)} a clear article target for a buyer question.`],
        source: "keyword_research",
      },
      upside: patch.upside || { low: 0, high: 0, positions: [] },
  ...
  async function readCompatUser(req) {
    ...
    if (runtime.SIR_BLOGGS_TRUST_BLAWGY_BEARER !== "1") return null;
    ...
    const email = String(payload.email || "local-blawgy-user@example.com").toLowerCase();
    return {
      id: `bearer:${payload.sub || email}`,
      email,
  ...
  function planPayload(site) {
    ...
    return {
      ...
      projection: { entriesCounted: plan.entries.length, high: 0, low: 0, note: "" },
  ...
  async function routeAccount(req, res, url, user) {
    ...
    const accountResult = await withAccount(user, (account) => {
      ...
      if (req.method === "POST" && url.pathname === "/test-connection") {
        const site = ensureSite(account, body.site, user);
        site.settings.blogType = body.blogType || site.settings.blogType;
        return {
          status: 200,
          payload: {
            success: true,
            message: "Connection successful.",
            collections: body.blogType === "framer" ? [{ id: "blog", name: "Blog" }] : undefined,
            projectName: body.blogType === "framer" ? site.site : undefined,
          },
        };
      }
      ...
      if (req.method === "POST" && url.pathname === "/research-competitors") {
        return {
          ...
          payload: {
            success: true,
            competitors: ["example competitor", "local directory", "regional marketplace"],
          },
        };
      }
      ...
      if (req.method === "POST" && ["/generate-image", "/generate-contextual-image"].includes(url.pathname)) {
        return {
          ...
          payload: {
            success: true,
            imageUrl: "/images/og-preview.png",
  ...
  function routeRootExtra(req, url, body, account, user) {
    ...
    const siteName = cleanSite(body.site || firstParam(url, "site") || firstParam(url, "website") || Object.keys(account.sites)[0]);
    ...
    if (req.method === "GET" && pathName === "/scrape-site") return { status: 200, payload: { success: true, productDescription: site.settings.businessDescription, faviconUrl: "/favicon.svg" } };
    ...
    if (req.method === "POST" && pathName === "/onboarding/complete") {
      site.settings = { ...site.settings, businessDescription: body.productDescription || site.settings.businessDescription, targetAudience: body.targetAudience || site.settings.targetAudience, site: site.site };
    ...
    if (req.method === "POST" && ["/cancel-subscription", "/switch-plan"].includes(pathName)) return { status: 200, payload: { success: true, subscription: subscription() } };
    ...
    if (req.method === "GET" && pathName === "/fetch-sitemap") return { status: 200, payload: { success: true, urls: [`https://${site.site}/`, `https://${site.site}/blog`] } };
    ...
    if (req.method === "GET" && pathName === "/framer/collections") return { status: 200, payload: { success: true, collections: [{ id: "blog", name: "Blog" }], projectName: site.site } };
    ...
    if (req.method === "DELETE" && pathName.startsWith("/sites/")) return { status: 200, payload: { success: true } };
    ...
    if (req.method === "POST" && pathName === "/save-article") {
      const blog = { id: id("blog"), _id: id("blog"), title: body.title || body.article?.title || "Saved article", blogStatus: body.status || "draft", publishDate: body.publishDate || iso(), blogContent: body.article?.html || body.article?.content || "", createdWith: "article-builder" };
    ...
    if (req.method === "PUT" && pathName.startsWith("/generate-blog/")) return { status: 200, payload: { success: true, blogStatus: "in_queue" } };
    if (req.method === "POST" && ["/save-post", "/update-publish-date", "/republish-article", "/publish-draft"].includes(pathName)) return { status: 200, payload: { success: true } };
    if (req.method === "PUT" && ["/bulk-delete-premises", "/cancel-blog-posting"].includes(pathName)) return { status: 200, payload: { success: true } };

    if (req.method === "GET" && pathName === "/gsc/data") return { status: 500, payload: { error: "Search Console data is not connected for this site." } };
    if (req.method === "GET" && pathName === "/site-settings") return { status: 200, payload: { success: true, settings: site.settings } };
    if (req.method === "GET" && pathName.startsWith("/gsc/sites/")) return { status: 200, payload: { success: true, sites: [`sc-domain:${site.site}`] } };
    if (req.method === "POST" && ["/gsc/connect", "/gsc/disconnect"].includes(pathName)) return { status: 200, payload: { success: true } };
    ...
    const result = await withAccount(user, (account, store) => {
      if (parts[0] === "save-keywords" && req.method === "POST") {
        ...
        const normalized = incoming
          .map((item) => (typeof item === "string" ? item : item?.kw || item?.keyword || item?.term || ""))
          .map((item) => String(item).trim())
          .filter(Boolean);
        saveSite.settings.keywords = Array.from(new Set([...(saveSite.settings.keywords || []), ...normalized]));
  ...
  function routePlan(req, parts, body, account, user) {
    ...
    if (req.method === "POST" && parts[2] === "generate") {
      const added = [];
      for (const cluster of defaultClusters(planSite.site, plan.config).slice(0, 2)) {
        if (plan.entries.some((entry) => entry.clusterLabel === cluster.label)) continue;
        const entry = makePlanEntry(cluster.pillarKeyword, { site: planSite.site, clusterLabel: cluster.label, source: "autopilot", publishDate: iso(7 + added.length * 7) });
    ...
    if (req.method === "GET" && parts[2] === "keyword-status") {
      const statuses = (planSite.settings.keywords || []).map((keyword) => ({
        keyword: typeof keyword === "string" ? keyword : keyword.keyword || keyword.kw,
        status: plan.entries.some((entry) => String(entry.keyword).toLowerCase() === String(keyword).toLowerCase()) ? "scheduled" : "available",
        volume: 500,
        kd: 20,
      }));
  ...
  function routeKeywordResearch(req, url, parts, body, account, user) {
    if (req.method === "POST" && parts[1] === "onboarding-preview") {
      ...
      return {
        ...
        payload: {
          success: true,
          clusters: defaultClusters(previewSite),
    ...
    if (req.method === "GET" && parts[2] === "clusters") {
      const clusters = defaultClusters(kwSite.site, kwSite.plan.config);
      return {
        status: 200,
        payload: {
          success: true,
  ...
  function routeProducts(req, parts, body, account, user) {
    ...
    if (req.method === "GET" && parts.length === 2) {
      ...
      return { status: 200, payload: { success: true, counts, products: prodSite.products, syncSettings: null } };
    }
    if (req.method === "GET" && parts[2] === "sync" && parts[3] === "status") return { status: 200, payload: { success: true, syncSettings: prodSite.productSync } };
    if (req.method === "POST" && parts[2] === "sync") return { status: 200, payload: { success: true, message: "Product sync queued.", sync: { added: 0, updated: prodSite.products.length, archived: 0 } } };
    if (req.method === "PUT" && parts[2] === "sync" && parts[3] === "settings") {
      prodSite.productSync = { ...prodSite.productSync, ...body };
      return { status: 200, payload: { success: true, syncSettings: prodSite.productSync } };
    }
    if (req.method === "POST" && parts.length === 2) {
      const product = { _id: id("prod"), status: "active", source: "manual", createdAt: iso(), updatedAt: iso(), ...body };
    ...
    if (req.method === "PUT" && product) {
      Object.assign(product, body, { _id: product._id, updatedAt: iso() });
      return { status: 200, payload: { success: true, product } };
    }
    if (req.method === "DELETE" && parts[2]) {
      removeById(prodSite.products, parts[2]);
      return { status: 200, payload: { success: true } };
    }
  ...
  function routePages(req, url, parts, body, account, user) {
    ...
    if (req.method === "POST" && parts[1] === "business-profiles") {
    ...
    if (["PUT", "DELETE", "POST"].includes(req.method) && parts[1] === "business-profiles" && parts[2]) {
      ...
      if (req.method === "DELETE") {
        removeById(pageSite.profiles, parts[2]);
        return { status: 200, payload: { success: true } };
      }
  ...
  function routeArticleBuilder(req, url, parts, body, account, user) {
    ...
    if (req.method === "PATCH" && parts[1] === "drafts" && parts[3] === "publish") {
      const draft = draftSite.drafts.find((row) => row.id === parts[2] || row._id === parts[2]);
      const blog = { id: id("blog"), _id: id("blog"), title: draft?.title || "Published article", blogStatus: "published", createdWith: "article-builder", publishDate: iso(), blogContent: draft?.articleData?.article || "", articleSummary: draft?.defineData?.prompt || "" };
      draftSite.blogs.unshift(blog);
      return { status: 200, payload: { success: true, blog } };
    }
  ...
  function routeWebhooks(req, url, parts, body, account, user) {
    const hookSite = ensureSite(account, body.site || firstParam(url, "site"), user);
    ...
    if (req.method === "POST" && webhook && parts[2] === "test") {
      webhook.lastTest = { event: body.event || "blog.created", success: true, testedAt: iso() };
      return { status: 200, payload: { success: true, status: 200, message: "Test event accepted." } };
    }
  ...
  function routeDutchie(req, parts, body, account, user) {
    ...
    if (req.method === "GET" && parts[2] === "status") return { status: 200, payload: { success: true, dutchie: { enabled: false, locations: dutchieSite.dutchie.locations || [] } } };
    if (req.method === "POST" && parts[2] === "connect") {
      const location = { ref: id("dutchie"), label: body.label || "Dutchie location", connectedAt: iso(), menuEmpty: false };
      dutchieSite.dutchie.locations.push(location);
      dutchieSite.dutchie.lastSyncedAt = iso();
      return { status: 200, payload: { success: true, message: "Dutchie connected.", dutchie: dutchieSite.dutchie, sync: { added: 0, updated: 0, archived: 0 }, menuEmpty: false } };
    }
    if (req.method === "DELETE" && parts[2] === "locations" && parts[3]) {
      dutchieSite.dutchie.locations = dutchieSite.dutchie.locations.filter((row) => row.ref !== parts[3]);
      return { status: 200, payload: { success: true } };
    }
    if (req.method === "POST" && parts[2] === "sync") {
      dutchieSite.dutchie.lastSyncedAt = iso();
      return { status: 200, payload: { success: true, added: 0, updated: 0, archived: 0, errors: [] } };
    }
