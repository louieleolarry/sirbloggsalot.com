...
1. `content-plan-overview-no-overlay.png`: Content Plan with onboarding/tour overlays closed, showing sidebar, workspace switcher area, strategy card, status counts, filters, tools/overflow menu labels, table/list or calendar controls, and at least one scheduled/generated/published row if available.
...
3. `article-builder-generated-editor.png`: Write or Article Builder after a draft has been generated, including the editor, SEO fields, image metadata, preview, save, schedule, publish, and regenerate controls.
4. `keyword-finder-results-add-to-plan.png`: Keyword Finder after a search or discovery run, including filters, metric columns, selected rows, saved batches if present, and the before/after state for Add to Plan.
5. `settings-cms-provider-error.png`: CMS Connect with a provider selected, required credential fields visible, Test setup result panel, and a safe validation/error state for missing or bad credentials. Do not submit real credentials.
...
8. `billing-plan-invoice-failure.png`: Billing page showing current plan/trial, plan change controls, period toggle, invoice detail, payment failure, cancel/reactivate, Stripe portal handoff, and Stripe Checkout and Link confirmation screens with payment details redacted.
9. `tracking-reports-active-states.png`: Search Console connected state, Rankings active Pro+ state, AI Mentions active state, Reports template/share/schedule/export, and SEO Analysis issues/detail views.
10. `team-workspace-switcher-open.png`: Workspace switcher open, exact Create/overflow menu labels and destinations, add-site flow, invite generation, pending invite row, revoke, accepted member row, role controls, remove member, and team activity.
11. `mobile-account-long-modal.png`: Phone-width screenshot of the sidebar/nav plus one long modal/form, preferably CMS connect, article edit, report schedule, support options/ticket, or invoice detail.
...
## Highest Priority Screenshot Pockets

1. Content Plan with no onboarding overlay, plus Strategy, tools/overflow menu, Bulk Schedule, Add Topics, grid/list, status filters, article drawer/card edit, generate/regenerate, schedule, publish/unpublish, delete, loading, empty, and error states.
2. Write or Article Builder from blank state through generated draft/editor, including template/type selection, brief fields, source inputs, product/location/CTA selectors, SEO metadata, image metadata, preview, save, publish, schedule, regenerate, disabled, loading, success, and validation states.
3. Topics or Keyword Finder, including search/discover fields, filters, metric columns, saved batches, topic detail, add-to-plan before/after, no-results, loading, provider error, exclude/regenerate controls, and pagination if present.
4. Settings CMS Connect, including platform dropdown options, WordPress/Webflow/Shopify/Wix/Ghost/Custom API field sets, required-credential copy, test connection, success, failure, draft-first/auto-publish, target collection/blog selection, and disconnect.
5. Products and inventory feed, including empty products, manual add/edit/delete, product card/list after one product exists, hide/show, filters, Inventory feed connect, sync result, failed sync, and disconnect.
6. Image Settings, including preset labels, toggles, product-image usage, guideline editor, generated image or prompt result, history, provider failure, and clear/reset behavior.
7. Billing inside the app, including current plan/trial card, upgrade/downgrade, period change, cancel, reactivate, invoice detail, payment failure, payment method, Stripe portal handoff, and Stripe Checkout and Link confirmation screens.
8. Tracking and Reports, including Search Console disconnected/connected states, Google connection flow, Rankings locked and active Pro+ states, AI Mentions locked and active states, Reports template/share/schedule/export, and SEO Analysis issue/detail views.
9. Team and Workspace, including workspace switcher open, Create/overflow menu labels and destinations, add site, switch site, invite generation, pending invite row, revoke, accepted member row, role change, remove member, and team activity.
10. Mobile, including sidebar/nav, settings tabs, Content Plan, Write/editor, Billing, and at least one long modal/form such as CMS connect, support ticket create, article edit, report schedule, or invoice detail.
