...
(async () => {
  ...
  try {
    const shellRoutes = ["/", "/features", "/pricing", "/affiliate", "/changelog", "/privacy", "/terms", "/signup", "/login", "/success?planId=price_local&amount=79&session_id=local", ...linkedCaseStudyRoutes];
    ...
    for (const route of ["/", "/features", "/pricing", ...linkedCaseStudyRoutes]) {
      ...
      assert.ok(signedInShell.body.includes('href="/account?view=plan" data-route="account" data-auth-account>Account</a>'), `${route} should send signed-in users an account home link`);
