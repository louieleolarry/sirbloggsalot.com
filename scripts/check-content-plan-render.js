...
const elements = {
  ...
  "[data-report-state-panel]": new FakeElement(),
  ...
  "[data-image-provider-state]": new FakeElement(),
  ...
  "[data-cms-validation]": new FakeElement(),
  ...
  "[data-inventory-state-panel]": new FakeElement(),
  ...
  "[data-billing-state-panel]": new FakeElement(),
  ...
  "[data-article-source-summary]": new FakeElement(),
  "[data-article-builder-output-state]": new FakeElement(),
...
function isoLocalDateDaysFromToday(days) {
  const date = new Date();
...
const defaultGridScheduledDate = isoLocalDateDaysFromToday(0);
    ...
    items: [
      {
        ...
        difficulty: "Easy",
        estimatedVisits: 44,
        scheduledDate: defaultGridScheduledDate,
        status: "scheduled",
      },
    ],
  },
...
const visibleCalendarDates = [0, 1, 2, 3].map(isoLocalDateDaysFromToday);
context.renderContentPlan({
  settings: { site: { publishingCadence: "3 per week" } },
  ui: { calendarMode: "grid" },
  contentPlan: {
    items: [
      { id: "scheduled-row", title: "Scheduled row", keyword: "scheduled", volume: 8100, estimatedVisits: 891, scheduledDate: visibleCalendarDates[0], status: "scheduled", difficulty: "Easy" },
      { id: "processing-row", title: "Processing row", keyword: "processing", scheduledDate: visibleCalendarDates[1], status: "processing" },
      { id: "generated-row", title: "Generated row", keyword: "generated", scheduledDate: visibleCalendarDates[2], status: "draft" },
      { id: "published-row", title: "Published row", keyword: "published", scheduledDate: visibleCalendarDates[3], status: "published" },
    ],
  },
});
assert.strictEqual(
...
assert.ok(firstCalendarCard.innerHTML.includes(`<time>${shortMonthDay(visibleCalendarDates[0])}</time>`));
...
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Inventory state"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Disconnected"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("0 manual"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("0 synced"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("credentials missing"));
...
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes('class="product-meta-grid"'));
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes("<dt>Category</dt>"));
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes("<dt>Price</dt>"));
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes("<dt>SKU</dt>"));
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes("<dt>Audience</dt>"));
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes("<dt>Source</dt>"));
assert.ok(elements["[data-products-list]"].children[0].innerHTML.includes("<dt>Visibility</dt>"));
assert.strictEqual(productFilterButtons.find((button) => button.dataset.productFilter === "all").classList.contains("is-active"), true);
...
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Tracking state"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Search Console"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("connected"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Rankings"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("1 keyword"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("AI Mentions"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("1 prompt"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Delivery"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("weekly"));
assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("/reports/report_local"));
...
vm.runInContext(
  `accountState.data = { id: "shared", ownerEmail: "owner@example.com", workspaceName: "Shared Workspace", ui: {}, contentPlan: { items: [] }, settings: {}, topics: [], products: [], locations: [], members: [{ id: "member-1", email: "editor@example.com", role: "editor", status: "active" }], invites: [{ id: "invite-1", email: "pending@example.com", role: "member", status: "pending" }], activityLog: [{ id: "activity-1", label: "Invite sent" }, { id: "activity-2", label: "Workspace switched" }], billing: {}, searchConsole: {}, rankings: {}, aiMentions: {}, supportTickets: [] };
...
assert.ok(elements["[data-dialog-body]"].innerHTML.includes('data-workspace-switch-summary'));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Selected workspace"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 workspaces"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 owner"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 member"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Add site workspace"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Team members"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 active"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Pending invites"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 pending"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Recent activity"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 updates"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-workspace-action-state"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes('data-workspace-action-detail="true"'));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Workspace actions"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Add site"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Invite team"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Role controls"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Owner access required"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 recent updates"));
...
vm.runInContext("showWorkspaceActionDetail();", context);
assert.strictEqual(elements["[data-dialog-title]"].textContent, "Workspace actions detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes('data-workspace-action-detail-card'));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Selected workspace"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Shared Workspace"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Workspace count"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 workspaces"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Role mix"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 owner"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Add site"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Owner access required"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Invite team"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 pending invite"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Role control"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 active member"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Team activity"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 recent updates"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact workspace selector order and multi-site add flow still need target screenshot evidence"));
...
assert.strictEqual(vm.runInContext('resolveRoute("/case-studies")', context), "case-studies");
...
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Payment method"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Visa ending 4242"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Retry date"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Aug 12"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Failure recorded"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Aug 10"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("data-billing-action-checklist"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Billing actions"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Plan change"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Portal handoff"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Payment retry"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Provider-backed"));
assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes('data-billing-action-detail="true"'));
context.handleDynamicAccountClick({
  target: {
    closest(selector) {
      if (selector === "[data-billing-action-detail]") return { dataset: { billingActionDetail: "true" } };
      return null;
    },
  },
});
assert.strictEqual(elements["[data-dialog-title]"].textContent, "Billing action detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-billing-action-detail-card"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Pro+"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Past due"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Visa ending 4242"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Card declined"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Aug 12"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Open billing portal"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Cancel disabled until payment state is resolved by owner action"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Stripe checkout and real payment-method management still need provider integration"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact billing portal and cancellation copy still needs target screenshot evidence"));
...
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Image provider"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Not tested"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("16:9"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("featured-only"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("0 saved"));

vm.runInContext(
  `accountState.data = { id: "owner", ownerEmail: "owner@example.com", workspaceName: "Owner", ui: {}, contentPlan: { items: [] }, settings: { images: { aspectRatio: "16:9", imageCadence: "key-sections", includeImages: true, useProductImages: true, samplePrompt: "Latest clean prompt", samplePreview: { title: "Local preview result", providerStatus: "provider-pending", aspectRatio: "16:9", imageCadence: "key-sections", stylePreset: "editorial", prompt: "Latest clean prompt", altText: "Latest local image preview" }, lastTestedAt: "2026-08-09T12:00:00.000Z", promptHistory: [{ id: "prompt-1", prompt: "Older editorial prompt", aspectRatio: "1:1", imageCadence: "featured-only", createdAt: "2026-08-08T12:00:00.000Z" }, { id: "prompt-2", prompt: "Latest clean prompt", aspectRatio: "16:9", imageCadence: "key-sections", createdAt: "2026-08-09T12:00:00.000Z" }] } }, products: [{ id: "product-1", name: "Premium Service Package" }], locations: [], members: [], invites: [], billing: {}, searchConsole: {}, rankings: {}, aiMentions: {}, supportTickets: [] };
accountState.workspaces = [{ id: "owner", role: "owner", selected: true }];
accountState.selectedWorkspaceId = "owner";`,
  context
);
...
assert.ok(elements["[data-image-details]"].children[0].innerHTML.includes('data-image-output-detail="latest"'));
...
assert.ok(elements["[data-image-details]"].children[1].innerHTML.includes('data-image-history-detail="0"'));
assert.ok(elements["[data-image-details]"].children[2].innerHTML.includes("Latest clean prompt"));
assert.ok(elements["[data-image-details]"].children[2].innerHTML.includes('data-image-history-detail="1"'));
assert.ok(elements["[data-image-details]"].children[0].innerHTML.includes("data-image-history-clear"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Provider pending"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Local preview result"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("16:9"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("key-sections"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("2 saved"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Preview status"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("provider-pending"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Style preset"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("editorial"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Alt text"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Latest local image preview"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Prompt"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Latest clean prompt"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Product images"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("enabled · 1 product"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("data-image-output-state"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Generated output"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Thumbnail"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Local preview result"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Save target"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Article preview only"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Provider-backed"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes('data-image-provider-detail="true"'));

context.handleDynamicAccountClick({
  target: {
    closest(selector) {
      if (selector === "[data-image-provider-detail]") return { dataset: { imageProviderDetail: "true" } };
      return null;
    },
  },
});
assert.strictEqual(elements["[data-dialog-title]"].textContent, "Image provider detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-image-provider-detail-card"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Provider pending"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Local preview result"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("editorial"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("16:9 · key-sections"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("enabled · 1 product"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 saved prompts"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Aug 9"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Test image prompt"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Final generation and asset-library selection remain provider-backed"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact generated thumbnails, failure rows, and asset-library selection still need target screenshot evidence"));

context.handleDynamicAccountClick({
  target: {
    closest(selector) {
      if (selector === "[data-image-output-detail]") return { dataset: { imageOutputDetail: "latest" } };
      return null;
    },
  },
});
assert.strictEqual(elements["[data-dialog-title]"].textContent, "Image output detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-image-output-detail-card"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Latest local image preview"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("provider-pending"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Article preview only"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Final generation stays pending until the image provider is connected"));

context.handleDynamicAccountClick({
  target: {
    closest(selector) {
      if (selector === "[data-image-history-detail]") return { dataset: { imageHistoryDetail: "0" } };
      return null;
    },
  },
});
assert.strictEqual(elements["[data-dialog-title]"].textContent, "Image prompt detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Older editorial prompt"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1:1"));
...
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Needs attention"));
assert.ok(elements["[data-image-provider-state]"].innerHTML.includes("Image provider rejected the prompt."));
...
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Provider validation"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Website URL required"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Platform required"));
assert.ok(elements["[data-cms-details]"].innerHTML.includes("No CMS configured"));

vm.runInContext(
  `accountState.data = { id: "owner", ownerEmail: "owner@example.com", workspaceName: "Owner", ui: {}, contentPlan: { items: [] }, settings: { cms: { websiteUrl: "https://sirbloggsalot.com", platform: "WordPress", status: "connected", hasCredentials: true, blogTarget: "News", collectionName: "Articles", lastTestedAt: "2026-08-10T12:00:00.000Z" } }, products: [], locations: [], members: [], invites: [], billing: {}, searchConsole: {}, rankings: {}, aiMentions: {}, supportTickets: [] };`,
  context
);
vm.runInContext("renderSettings(accountState.data);", context);
assert.strictEqual(elements['[data-account-action="connect-cms"]'].disabled, false);
...
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Provider validation"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("WordPress"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Provider test pending"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Draft first"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Credentials"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("configured"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Target"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("News"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Collection"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Articles"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Last test"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Aug 10"));
...
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Needs attention"));
assert.ok(elements["[data-cms-validation]"].innerHTML.includes("Website URL and platform are required before connecting."));
...
assert.ok(elements["[data-cms-details]"].children[0].innerHTML.includes('data-cms-provider-detail="true"'));
context.handleDynamicAccountClick({
  target: {
    closest(selector) {
      if (selector === "[data-cms-provider-detail]") return { dataset: { cmsProviderDetail: "true" } };
      return null;
    },
  },
});
assert.strictEqual(elements["[data-dialog-title]"].textContent, "CMS provider detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-cms-provider-detail-card"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("WordPress"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("WordPress site URL"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Application password or publishing token"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Credential metadata missing"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Website URL and platform are required before connecting."));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Missing or rejected credentials stay visible as a provider error"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Secret values are never displayed"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact provider-form validation copy still needs target screenshot evidence"));
...
vm.runInContext(
  `accountState.data = { id: "owner", ownerEmail: "owner@example.com", workspaceName: "Owner", ui: {}, contentPlan: { items: [] }, settings: {}, products: [{ id: "manual-1", name: "Manual service", source: "manual", hidden: false, sku: "MAN-1" }, { id: "synced-1", name: "Synced service", source: "inventory-feed", hidden: false, sku: "SYNC-1" }, { id: "hidden-1", name: "Hidden synced service", source: "inventory-feed", hidden: true, sku: "SYNC-HIDDEN" }], locations: [], members: [], invites: [], billing: {}, searchConsole: {}, rankings: {}, aiMentions: {}, inventoryFeed: { status: "connected", retailerName: "Test Catalog", accountId: "retailer-123", hasCredentials: true, syncMode: "manual", lastSyncedAt: "2026-08-12T12:30:00.000Z" }, supportTickets: [] };`,
  context
);
vm.runInContext("renderSettings(accountState.data);", context);
assert.strictEqual(elements['[data-account-action="connect-inventory"]'].disabled, false);
...
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Inventory state"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Connected"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Test Catalog"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("retailer-123"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("credentials configured"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Feed"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Account"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Sync mode"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("manual"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Manual products"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("1"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Synced products"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("2"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Hidden products"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("1"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Last sync"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Aug 12"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Latest SKU"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("SYNC-1"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes('data-inventory-feed-detail="true"'));
context.handleDynamicAccountClick({
  target: {
    closest(selector) {
      if (selector === "[data-inventory-feed-detail]") return { dataset: { inventoryFeedDetail: "true" } };
      return null;
    },
  },
});
assert.strictEqual(elements["[data-dialog-title]"].textContent, "Inventory feed detail");
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-inventory-feed-detail-card"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Test Catalog"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Connected"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("retailer-123"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("credentials configured"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("manual"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 manual"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("2 synced"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 hidden"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Aug 12"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("SYNC-1"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Sync inventory products"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Real inventory API sync remains provider-backed"));
assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact product add/edit and synced-product detail states still need target screenshot evidence"));
...
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Needs attention"));
assert.ok(elements["[data-inventory-state-panel]"].innerHTML.includes("Provider credentials need review"));
...
accountState.loading = false;`,
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Shopify Admin API"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-cms-provider-checklist"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Provider checklist"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Store URL"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Admin account"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Blog or collection target"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Secret handling"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Required credential"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Admin API access token"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Missing or rejected credentials stay visible as a provider error"));
...
accountState.productSearch = "";`,
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-inventory-connect-checklist"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Connection setup"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Credential state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Credentials missing"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Secret handling"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Secret values are never displayed"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Provider-backed"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Real inventory API sync remains provider-backed"));
...
accountState.data = { id: "owner", ownerEmail: "owner@example.com", workspaceName: "Owner", ui: {}, contentPlan: { items: [] }, settings: {}, products: [], locations: [], members: [], invites: [], billing: { plan: "Pro", status: "trial", billingPeriod: "monthly", invoices: [] }, searchConsole: {}, rankings: {}, aiMentions: {}, supportTickets: [] };`,
  ...
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Billing state"));
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Pro+"));
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("local-checkout"));
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("invoice_checkout"));
...
accountState.selectedWorkspaceId = "member-workspace";`,
  ...
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Past due"));
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("payment-failed"));
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("invoice_failed"));
  assert.ok(elements["[data-billing-state-panel]"].innerHTML.includes("Card declined"));
  await context.handleDynamicAccountClick({
    target: {
      closest(selector) {
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-billing-invoice-detail"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Invoice detail"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Payment handoff"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Stripe invoice payment remains provider-backed"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Retry or payment-method changes happen in the billing portal"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Failure reason"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Card declined"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Local invoice URL"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("/account?view=billing&amp;invoice=invoice_failed") || elements["[data-dialog-body]"].innerHTML.includes("/account?view=billing&invoice=invoice_failed"));
...
accountState.mentionsSearch = "";`,
  ...
  vm.runInContext(
    `accountState.data = { id: "owner", ownerEmail: "owner@example.com", workspaceName: "Owner", ui: {}, contentPlan: { items: [] }, settings: {}, products: [], locations: [], members: [], invites: [], billing: { plan: "Pro+", status: "active" }, searchConsole: { status: "connected", lastSyncedAt: "2026-08-09T12:00:00.000Z", clicks: 120, impressions: 2400, indexedPages: 7 }, rankings: { lastSyncedAt: "2026-08-08T12:00:00.000Z", keywords: [{ keyword: "alpha", position: 3, change: 2, url: "/blog/alpha" }] }, aiMentions: { lastSyncedAt: "2026-08-07T12:00:00.000Z", mentions: [{ source: "ChatGPT", model: "GPT-4o", prompt: "best alpha", status: "mentioned" }] }, reports: { template: "executive", lastExportedAt: "2026-08-10T12:00:00.000Z", sharing: { shareUrl: "/reports/report_local", shareCreatedAt: "2026-08-09T12:00:00.000Z" }, schedule: { enabled: true, cadence: "monthly", template: "executive", recipients: ["owner@example.com", "team@example.com"], lastScheduledAt: "2026-08-11T12:00:00.000Z" } }, supportTickets: [] }; renderReports(accountState.data);`,
    context
  );
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Share link"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("/reports/report_local"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Schedule"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("monthly to 2 recipients"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Last export"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Aug 10"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Search synced"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Aug 9"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Rankings synced"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Aug 8"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Mentions synced"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Aug 7"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("data-report-action-state"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Reports actions"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Export CSV"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Ready - last Aug 10"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Copyable local link"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Schedule delivery"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("monthly to 2 recipients"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes("Provider-backed"));
  assert.ok(elements["[data-report-state-panel]"].innerHTML.includes('data-report-action-detail="true"'));
  context.handleDynamicAccountClick({
    target: {
      closest(selector) {
        if (selector === "[data-report-action-detail]") return { dataset: { reportActionDetail: "true" } };
        return null;
      },
    },
  });
  assert.strictEqual(elements["[data-dialog-title]"].textContent, "Reports actions detail");
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-report-action-detail-card"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Executive digest"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("connected"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 keyword"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("1 prompt"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("/reports/report_local"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("monthly to 2 recipients"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Aug 10"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Export local CSV"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Copyable local link"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Live analytics and scheduled email delivery still need provider integrations"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact export, share, and schedule modal copy still needs target screenshot evidence"));
...
vm.runInContext(
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-report-schedule-checklist"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Delivery setup"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Schedule state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Disabled locally"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Local schedule metadata only"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Provider-backed"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Email delivery remains provider-backed"));
...
accountState.selectedTopicIds = new Set();`,
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-lifecycle"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-action-state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Action state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes('data-article-lifecycle-detail="plan-card"'));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated Aug 12"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Updated Aug 13"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("No active issue"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Lifecycle"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Scheduled"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Draft first"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("CMS connected"));
  ...
  await context.handleDynamicAccountClick({
    target: {
      closest(selector) {
        if (selector === "[data-article-lifecycle-detail]") return { dataset: { articleLifecycleDetail: "plan-card" } };
        return null;
      },
    },
  });
  assert.strictEqual(elements["[data-dialog-title]"].textContent, "Article lifecycle detail");
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-lifecycle-detail-card"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Calendar card article"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Calendar body"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Calendar SEO"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Aug 24 at 09:30"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated Aug 12"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Updated Aug 13"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Shows progress while the protected request runs"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Exact drawer loading and success states still need target screenshot evidence"));
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-lifecycle-confirm"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Lifecycle confirmation"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-confirm-action-state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Action readiness"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Shows progress while the protected request runs"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Protected request errors appear in the account status line"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Move back to draft"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Public URL removed locally"));
  await vm.runInContext("accountState.dialogSubmit({})", context);
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-lifecycle-confirm"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-confirm-action-state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Delete from active table"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Cannot be undone locally"));
  await vm.runInContext("accountState.dialogSubmit({})", context);
  ...
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Lifecycle confirmation"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-confirm-action-state"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Publish to public blog"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Backend validation required"));
  await vm.runInContext("accountState.dialogSubmit({})", context);
  ...
  vm.runInContext(
    `accountState.data = {
      ...
      writeDraft: {
        ...
        canonicalUrl: "https://sirbloggsalot.com/blog/generated-builder-post",
        featuredImageUrl: "https://sirbloggsalot.com/assets/generated-builder-post.jpg",
        featuredImageAlt: "Generated builder post image",
        preview: "Generated local article preview",
        previewStatus: "provider-pending",
        generatedAt: "2026-08-14T12:00:00.000Z",
        status: "draft"
      }
  ...
  assert.strictEqual(elements["[data-article-generated-actions]"].hidden, false);
  assert.strictEqual(elements["[data-article-source-summary]"].hidden, false);
  assert.strictEqual(elements["[data-article-builder-output-state]"].hidden, false);
  assert.ok(elements["[data-article-source-summary]"].innerHTML.includes("Generated article"));
  assert.ok(elements["[data-article-source-summary]"].innerHTML.includes("Generated"));
  assert.ok(elements["[data-article-source-summary]"].innerHTML.includes("Aug 20 at 09:30"));
  assert.ok(elements["[data-article-source-summary]"].innerHTML.includes("/blog/generated-builder-post"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Output state"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Generated Aug 14"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Source Generated"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("No active issue"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("data-article-editor-qa"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Editor QA"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Save target"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Validation"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Image"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("SEO"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Provider handoff"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("data-article-editor-fields"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Editor fields"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Body"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("SEO title"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Meta description"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Featured image"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes("Preview"));
  assert.ok(elements["[data-article-builder-output-state]"].innerHTML.includes('data-article-generation-detail="true"'));
  context.handleDynamicAccountClick({
    target: {
      closest(selector) {
        if (selector === "[data-article-generation-detail]") return { dataset: { articleGenerationDetail: "true" } };
        return null;
      },
    },
  });
  assert.strictEqual(elements["[data-dialog-title]"].textContent, "Article generation detail");
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("data-article-generation-detail-card"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated builder post"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated body"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated SEO title"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated meta description"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Generated builder post image"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Aug 20 at 09:30"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Provider pending"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Local only; CMS pending"));
  assert.ok(elements["[data-dialog-body]"].innerHTML.includes("Provider-generated long-form body fidelity still needs target screenshot evidence"));
