...
const appShellRoutes = new Set(["/", "/features", "/pricing", "/login", "/signup", "/success", "/affiliate", "/changelog", "/privacy", "/terms", "/account"]);
...
function personalizeAppShellHtml(html, user) {
  ...
  return html
    ...
    .replace(
      ...
      '<a class="login-link" href="/account?view=plan" data-route="account" data-auth-account>Account</a>'
    )
    .replace(
...
async function serveStatic(req, res) {
  ...
  if (req.method !== "HEAD" && contentType.includes("text/html")) {
    const user = await getSessionUser();
    body = personalizeAppShellHtml(data.toString("utf8"), user);
  }
