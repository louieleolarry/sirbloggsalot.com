...
const caseStudies = {
  natureva: {
    title: "Natureva",
    summary: "Ecommerce content grew US search traffic while supporting international rankings.",
    category: "Ecommerce",
    context: "A product-led site needed more useful buying guides, category context, and answer-ready educational articles.",
    result: "1,962% traffic growth",
    resultCopy: "Six months of consistent publishing produced major US traffic growth and #1 rankings in Australia.",
  },
  foodbuddy: {
    title: "MyFoodBuddy",
    summary: "A new app domain turned consistent content into impressions, trials, and paid customers.",
    category: "App",
    context: "A young software site needed indexed answers before customers compared alternatives.",
    result: "3.65M impressions",
    resultCopy: "Content velocity helped the domain reach millions of impressions and 71 paying customers.",
  },
  "kush-groove": {
    title: "Kush Groove",
    summary: "Local business content moved a quiet blog into meaningful organic discovery.",
    category: "Local business",
    context: "A local retailer needed service-area and product-aware content that could rank beyond brand searches.",
    result: "321K impressions",
    resultCopy: "Six months of structured publishing took the blog from zero traffic to 321K impressions.",
  },
};
...
function renderSettings(account) {
  ...
  if (imageDetails) {
    ...
    if (!images.samplePrompt) {
      ...
      const previewHtml = preview.prompt
        ...
        : "";
      row.innerHTML = `<div>${previewHtml}<strong>Latest image prompt</strong><p>${escapeHtml(imageMeta)} · ${escapeHtml(images.samplePrompt)}</p></div><button type="button" data-image-history-clear data-account-action="clear-image-tests" ${readOnly ? "disabled" : ""}>Clear history</button><span>${images.lastTestedAt ? formatDateLabel(images.lastTestedAt.slice(0, 10)) : "Not tested"}</span>`;
      imageDetails.appendChild(row);
      (images.promptHistory || []).forEach((entry) => {
        ...
        historyRow.innerHTML = `<div><strong>${escapeHtml(entry.stylePreset || "Image prompt")}</strong><p>${historyMeta ? `${escapeHtml(historyMeta)} · ` : ""}${escapeHtml(entry.prompt || "")}</p></div><span>${entry.createdAt ? formatDateLabel(entry.createdAt.slice(0, 10)) : "Saved"}</span>`;
  ...
  if (cmsDetails) {
    ...
    if (!cms.platform && !cms.websiteUrl) {
      ...
      row.innerHTML = `<div><strong>${escapeHtml(cms.platform || "Platform not selected")}</strong>${statusBadge}<p>${escapeHtml(cms.websiteUrl || "No website URL")} · ${escapeHtml(cms.status || "disconnected")} · Local setup only · real publisher pending · ${cms.hasCredentials ? "credentials configured" : "credentials missing"}${cmsTargetMeta ? ` · ${escapeHtml(cmsTargetMeta)}` : ""}${cms.lastTestedAt ? ` · tested ${formatDateLabel(cms.lastTestedAt.slice(0, 10))}` : ""}${cms.lastError ? ` · ${escapeHtml(cms.lastError)}` : ""}</p></div><span>${cms.draftFirst === false ? "Auto-publish" : "Draft first"}</span>`;
...
function renderProducts(account) {
  ...
  products.forEach((product) => {
    ...
    row.innerHTML = `<button class="product-detail-link" type="button" data-product-detail="${escapeAttribute(product.id)}"><span class="product-row-heading"><strong>${escapeHtml(product.name)}</strong><span class="product-badges">${badges}</span></span><p>${details.map((detail) => escapeHtml(detail)).join(" · ")}</p></button><button type="button" data-product-edit="${escapeAttribute(product.id)}" ${readOnlyDisabled}>Edit</button><button type="button" data-product-toggle="${escapeAttribute(product.id)}" ${readOnlyDisabled}>${product.hidden ? "Show" : "Hide"}</button><button type="button" data-product-delete="${escapeAttribute(product.id)}" ${readOnlyDisabled}>Delete</button>`;
...
function renderBilling(account) {
  ...
  if (billingStatusLabel) billingStatusLabel.textContent = status === "past_due" ? "Past due" : status.charAt(0).toUpperCase() + status.slice(1);
  if (billingSummary) {
  ...
  try {
    ...
    if (action === "connect-inventory") {
      ...
      showAccountForm({
        ...
        body: "Store retailer metadata only. Secret values are accepted to mark credentials configured, but the local dashboard does not return or display them.",
    ...
    if (action === "connect-cms") {
      ...
      showAccountForm({
        title: "Connect CMS",
        body: `${platformMeta.body} Secret values are accepted to mark credentials configured, but the local dashboard does not return or display them.`,
    ...
    if (action === "schedule-report") {
      ...
      showAccountForm({
        ...
        body: "Save a local report delivery schedule. Email delivery remains pending provider integration.",
...
async function openInvoiceDetails(invoiceId) {
  ...
  const lines = [
    ...
    invoice.failureReason ? `Failure reason: ${invoice.failureReason}` : "",
    invoice.hostedInvoiceUrl ? `Local invoice URL: ${invoice.hostedInvoiceUrl}` : "",
  ].filter(Boolean);
  showAccountDialog(`Invoice ${invoice.id || invoiceId}`, lines.join("\n"));
}
